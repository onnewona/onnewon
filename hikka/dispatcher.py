...
class CommandDispatcher:
    ...
    async def command_exc(self, e, message: Message, prefix: str):
        ...
        if not self._db.get(main.__name__, "inlinelogs", True):
            try:
                txt = f"◍ <b>Eu</b> <code>{utils.escape_html(prefix)}{utils.escape_html(message.message)}</code><b> - bilan xato yuzaga keldi!</b>"
        ...
        try:
            ...
            txt = (
                f"<b>◍ <b>Eu</b> <code>{utils.escape_html(prefix)}{utils.escape_html(message.message)}</code><b> - bilan xato yuzaga keldi!</b>\n\n"
                f"<b>◍ <b>Xato koʻrinishi:</b> </b>\n<code>{exc}</code>"
            )
            await (message.edit if message.out else message.reply)(txt)
        except Exception:
            pass
