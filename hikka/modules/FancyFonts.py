...
FONT = [
    "ᴀʙᴄᴅᴇғɢʜɪᴊᴋʟᴍɴᴏᴘǫʀsᴛᴜᴠᴡxʏᴢ𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿𝟶ABCDEFGHIJKLMNOPQRSTUVWXYZ",
]
FONTZ = [
    "𝗮𝗯𝗰𝗱𝗲𝗳𝗴𝗵𝗶𝗷𝗸𝗹𝗺𝗻𝗼𝗽𝗾𝗿𝘀𝘁𝘂𝘃𝘄𝘅𝘆𝘇𝟭𝟮𝟯𝟰𝟱𝟲𝟳𝟴𝟵𝟬𝗔𝗕𝗖𝗗𝗘𝗙𝗚𝗛𝗜𝗝𝗞𝗟𝗠𝗡𝗢𝗣𝗤𝗥𝗦𝗧𝗨𝗩𝗪𝗫𝗬𝗭",
]
...
class FancyFontsMod(loader.Module):
    ...
    strings = {
		"name": "AutoWord", 
		"no_args": "🐝 <b>Please, write under command...</b>"
	}
	        
    async def ttelegacmd(self, message: Message) -> None:
        """- <text> automatic translate in a coding font"""
        args = utils.get_args_raw(message)
        if not args:
            await utils.answer(message, self.strings("no_args"))
        else:
            for i in FONT:
                await message.reply(f"{str.translate(args, str.maketrans(BASE, i))}")

      
    async def tboldcmd(self, message: Message) -> None:
        """- <text> automatic translate in a bold font"""
        args = utils.get_args_raw(message)
        if not args:
            await utils.answer(message, self.strings("no_args"))
        else:
    ...
    async def tcalibcmd(self, message: Message) -> None:
        """- <text> automatic translate in a beautiful font"""
        args = utils.get_args_raw(message)
        if not args:
            await utils.answer(message, self.strings("no_args"))
        else:
    ...
    async def tcodecmd(self, message: Message) -> None:
        """- <text> automatic translate in a beautiful font"""
        args = utils.get_args_raw(message)
        if not args:
            await utils.answer(message, self.strings("no_args"))
